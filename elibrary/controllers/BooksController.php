	...
	<dl>
		...
		$html .= "<span class=\"val_" . Book::getRatingInt($rating) . "\">" . Book::getRatingText($rating) . "</span>";
			...
			<!--
				...
				val_5 => Ottimo
			--> */
		$html .= "</dd><dt class=\"price\">Prezzo</dt>
		<dd class=\"price\">" . Application::priceToString($book->getPrice()) . " <span class=\"taxes\">(IVA incl.)</span></dd>
		</dl>";
		$params = array(
			"id" => $book->getId()
		); 
		if($addbtn)
		{
			$html .= "<a class=\"addtocart\" role=\"button\" href=\"";
			$html .= FrontController::getUrl("cart", "add", $params);
			$html .= "\">Aggiungi al carrello</a>";
		}
		$html .= "<a class=\"moreinfo\" role=\"button\" href=\"";
